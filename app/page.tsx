...
import { Disclosure }  from '@headlessui/react';   // still here
import { ChevronDown } from 'lucide-react';        // still here
        ...
        <div className="relative z-10 flex flex-col justify-center px-8 md:px-16 py-16 max-w-6xl">
          {typed.split('\n').map((line, i) => (
            <h1 key={i} className="text-5xl md:text-7xl font-bold leading-tight text-white">
              {i === 1 && line.startsWith('Zero') ? (
                <>
                  <span className="text-[#2dd4bf]">Zero</span>{line.slice(4)}
                </>
              ) : line}
            </h1>
          ))}
          ...
          <div className="mt-10">
            <Link
              ...
              className="inline-block px-8 py-3 rounded-full text-sm font-semibold text-white shadow
                         bg-gradient-to-r from-cyan-400 via-teal-400 to-fuchsia-500 hover:brightness-110"
            >
              Get&nbsp;Started&nbsp;For&nbsp;Free
            </Link>
            <p className="mt-2 ml-2 text-sm text-gray-200">No credit card required</p>
          </div>
        </div>

        <div className="relative z-10 pb-8 flex justify-center w-full">
          <button onClick={scrollToPerf} className="text-4xl animate-bounce text-gray-300">
            ↓
          </button>
        </div>
      </section>
    ...
    <section className="py-24 px-4 md:px-12 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto text-center space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-white">
            <StatBox value="98%" label="Win Rate" />
            <StatBox value="4.36" label="Profit Factor" />
            <StatBox value="30 Days" label="Free Trial" />
          </div>

          <Link
            href="/auth?screen_hint=signup"
            className="mt-16 inline-block text-black font-semibold px-10 py-4 rounded-full
     ...
     <section className="px-4 md:px-12 pb-32 backdrop-blur-sm">
      ...
      {showTop && (
        <button
          ...
          className="fixed bottom-8 right-8 p-3 rounded-full text-white shadow-lg
                     bg-gradient-to-r from-blue-500 via-teal-400 to-cyan-300 hover:brightness-110"
